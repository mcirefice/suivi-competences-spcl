...
function seedQCM() {
  ...
  const avance = [
    ["S&P1", "Une autre carte utilise un CAN de 12 bits (au lieu de 10 bits sur l'Arduino Uno). Combien de niveaux de codage possibles cela représente-t-il ?",
     ...
     "N = 2ⁿ = 2¹² = 4096 niveaux, numérotés de 0 à 4095. Attention à ne pas confondre le NOMBRE de niveaux (4096) avec la valeur MAXIMALE codée (4095)."],
    ["S&P1", "Pour ce CAN 12 bits, calibré sur Vmax = 3,3V, quel est le quantum (arrondi à 0,1 mV) ?",
     "0,8 mV", "3,3 mV", "1,2 mV", "4,89 mV", "A",
     "q = Vmax / (2ⁿ - 1) = 3300 / 4095 ≈ 0,806 mV, soit environ 0,8 mV. Attention, 4,89 mV est le quantum du CAN 10 bits de l'Arduino Uno sur 5V vu en cours — ce n'est pas la même carte ici !"],
    ["S&P1", "Entre le CAN 10 bits de l'Arduino Uno (sur 5V) et un CAN 8 bits (sur 5V également), lequel est le plus précis ?",
     ...
     "Plus n est grand, plus 2ⁿ est grand, donc plus le quantum q = Vmax/(2ⁿ-1) est petit : le CAN 10 bits distingue des écarts de tension plus fins que le CAN 8 bits, à Vmax identique."],
    ["S&P1", "Un signal audio contient des fréquences jusqu'à fmax = 20 kHz (limite de l'audition humaine). D'après Shannon, la fréquence d'échantillonnage doit être strictement supérieure à :",
     ...
     "fe > 2 × fmax = 2 × 20 = 40 kHz. C'est d'ailleurs pour rester au-dessus de ce seuil, avec une marge de sécurité, que les CD audio utilisent 44,1 kHz plutôt que tout juste 40 kHz."],
    ["S&P1", "Dans le code de comparaison de résolution vu en AE02, l'instruction (brut >> 4) << 4 sert à dégrader une mesure 10 bits en 6 bits. Que fait concrètement l'opérateur >> 4 ?",
     ...
     "Un décalage à droite de 4 bits (>>4) revient à diviser par 2⁴ = 16 en tronquant : les 4 bits de poids faible (les moins significatifs) sont perdus, ce qui dégrade la résolution."],
    ["S&P2", "Une sonde Pt100 (R ≈ 107,79 Ω à 20°C) est conditionnée par un pont diviseur avec Rcond = 150 Ω et E = 5V, selon Vout = E × Rcond/(Rcond+Rcapteur). Quelle est la tension de sortie approximative ?",
     ...
     "Vout = 5 × 150 / (150 + 107,79) = 5 × 150 / 257,79 ≈ 2,91 V."],
    ...
    ["S&P2", "Pour déterminer la chaleur massique d'un métal par la méthode des mélanges, quelle grandeur n'est PAS nécessaire ?",
     ...
     "La couleur du métal n'intervient dans aucune formule de calorimétrie — seules les masses, les températures et la capacité thermique du calorimètre sont nécessaires. Un bon réflexe d'esprit critique : identifier les grandeurs réellement utiles avant de se lancer dans un calcul."],
    ["S&P2", "En passant de 3 à 5 points de mesure répartis sur toute la plage de température (même précision du thermomètre de référence), l'écart entre pente max et pente min obtenu par le faisceau de droites va généralement :",
     ...
     "Plus les points extrêmes utilisés pour tracer le faisceau sont éloignés l'un de l'autre (grande plage de température couverte), moins l'incertitude ±u(T) sur chaque point pèse relativement sur la pente calculée : l'encadrement se resserre."],
    ["S&P2", "Contrairement à la Pt100 (résistance qui augmente avec la température), une thermistance CTN a une résistance qui :",
     ...
     "CTN signifie « Coefficient de Température Négatif » : sa résistance diminue quand la température augmente — un comportement inverse de celui de la Pt100."],
  ];
